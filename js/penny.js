addLayer("p", {
    ...
    gainExp() { // Calculate the exponent on main currency from bonuses
        ...
        if (inSluggishLayer(1)) exp = exp.pow(.5)
    ...
    directMult() {
        ...
        if (inSluggishLayer(1)) ret = ret.pow(.5)
    ...
    upgrades: {
        ...
        21: {
            ...
            fullDisplay:() => {
                ...
                let description = () => {
                    if (inSluggishLayer(1)) return "Increase point gain by 50% per achievement"
                    if (!hasUpg("e", 44)) {
            ...
            effect:() => {
                if (hasMilestone("a", 5) && !inSluggishLayer(1)) return upgEff("p", 35)
        ...
        22: {
            ...
            description() {
                let ret = "Multiply point gain by (1 + Pennies/100)<sup>.9</sup>"
                if (hasAchievement("a", 64)) ret = "Multiply point gain by (1 + Pennies)<sup>.9</sup>"
                if (this.effect().gte(1e60)) ret += " (softcapped)"
                return ret
            },
            cost: new Decimal("100"),
            effect() {
                let ret = player.p.points.div(100)
        ...
        23: {
            description() {
                ...
                if (upgrade23LimitExp() > 1) ret += ` but cap Points to 100 * (Pennies<sup>${format(upgrade23LimitExp())}</sup> + 1)`// * 100"
                else ret += " but limit Points to 100 * (Pennies + 1)"//100 + Pennies * 100"

                return ret
            },
            fullDisplay() {
                let title = "<h3>We Need Bigger Pockets</h3>"
                let desc, eff
                if (player.sys.lockWNBP) { 
                    desc = "Locked by the toggle in the 5th System milestone"
                    eff = "" 
                } else { 
                    desc = this.description()
                    eff = `Currently: capped at ${formatWhole(upgrade23Limit())} points<br>` 
                }
                let cost = "Cost: 250 pennies"
                return `${title}<br>${desc}<br>${eff}<br>${cost}`
            },
        ...
        35: {
            ...
            cost: 2e7,
            effect() {
                if (inSluggishLayer(1)) return new Decimal(1 + .5 * player.a.achievements.length).pow(.2)
                if (!hasUpg("e", 44)) {
        ...
        42: {
            ...
            effect() {
                ...
                return player.p.investment2.points.add(1).pow(this.effExponent())
            },
            effExponent() {
                let ret = .4
                if (tmp.s.challenges[11].unlocked) ret = Decimal.add(ret, challengeEffect("s", 11))
                return ret
            },
            fullDisplay() {
        ...
        53: {
            ...
            description:() => {
                if (!player.shiftDown) {
                    let ret = "Multiply investment gain<sup>*</sup> by 1 + log10(1 + Penny Expansions)"
                    if (!hasAchievement("a", 112)) ret += "<sup>.5</sup>"
                    return ret
                }
            ...
            effect:() => {
                let ret = player.e.penny_expansion.points.add(1).log10()
                if (!hasAchievement("a", 112)) ret = ret.pow(.5)
    ...
    buyables: {
        ...
        12: {
            ...
            display() {
                ...
                let gain = tmp.p.buyables[12].gain
                let ret = `Investing your investment will earn you ${format(gain)} expansion investment.`
                if (gain.gte(this.softcap())) {
                    ret = `Your expansion investment gain is currently <b><h3>softcapped</b></h3>. Excess gain past ${format(this.softcap())}`
                        + ` is raised to a power of .4 and capped at ${format(this.gainLimit())}<br><br>` + ret
                }
                return ret
            },
            canAfford() {
            ...
            gain() {
                ...
                if (ret.gte(softcapStart)) {
                    ...
                    ret = softcapStart.add(excess.pow(.4))
                }

                return ret.min(tmp.p.buyables[12].gainLimit)
            },
            gainLimit() { 
                let ret = this.hardcap().div(3) 
                return ret
            },
            buy() {
                let cap = tmp.p.buyables[12].hardcap
                if (player.p.investment2.points.gte(cap)) {
                ...
                if (tmp.a.achievements[102].unlocked && player.a.achievements.indexOf("102") == -1 && inChallenge("s", 11) && gain.gte(1337)) {
                    player.a.achievements.push("102")
                    doPopup("achievement", tmp.a.achievements[102].name, "Achievement Unlocked!", 3, tmp.a.color)
                }
                player.p.investment2.points = player.p.investment2.points.add(gain).min(cap)
        ...
        21: {
            ...
            cost() {
                let baseCost = 2e5
                let base = 2
                let exp = getBuyableAmount("p", 21).div(buyableEffect("p", 22)).pow(2)
        ...
        22: {
            ...
            display() {
                ...
                let effFormulaBase = "<b><h3>Effect Formula (softcap begins at effect of "
            ...
            effect() {
                if (!this.unlocked()) return decimalOne
                let base = player.e.penny_expansion.points.add(1).log(4).div(8)
        ...
        23: {
            title: "Education III",
            baseCost() {
                let ret = !hasAchievement("a", 112) ? new Decimal(".15") : new Decimal(".1")
                return ret
            },
            cost() {
                let base = new Decimal("2")
                let exp = new Decimal(getBuyableAmount("p", 23))
                return this.baseCost().mul(base.pow(exp))
            },
            coefficient:() => {
                let ret = .25
            ...
            display() {
                ...
                let effFormula1 = `(1 + ${format(coeff)}x)<sup>(1 + ${format(coeff)}x)</sup><br>`
                ...
                let costFormula = `<b><h3>Cost Formula:</h3></b><br>${format(this.baseCost())}*2^x`
    ...
    update(diff) {
        ...
        if (hasMilestone("s", 6)) {
            if (inChallenge("s", 11) || inChallenge("s", 12)) {
                ...
                tmp.p.buyables[11].gain = layers.p.buyables[11].gain()
            }
            let percent = tmp.s.stored_dollars.effects[5].mul(.01)
        ...
        "Upgrades": {
            content: [
                ...
                ["display-text", function() {
                    ...
                    if (tmp.p.passiveGeneration > 0) ret += `You are gaining ${format(tmp.p.passiveGeneration*tmp.p.resetGain)}
                        pennies per second`
                    return ret
                }],
                "blank",
            ...
            content: [
                ...
                ["display-text",
                    function(){
                        let ret = ""
                        if (tmp.p.passiveGeneration > 0) ret += `You are gaining ${format(tmp.p.passiveGeneration*tmp.p.resetGain)}
                            pennies per second<br>`
                        if (hasMilestone("s", 6)) ret += `You are gaining ${format(tmp.s.stored_dollars.effects[5])}%
                            of investment on reset per second<br>`
                        ret += `<br>You have 
        ...
        info: {
            ...
            "Taxes": {
                content: [
                    ...
                    + "<br> where x = max([Best Pennies] / 2, [Current Pennies]), "
                    + "PTS (pennyTaxStart) = " + format(pennyTaxStart()) + ", and PTE (pennyTaxExponent) = " + format(pennyTaxExp(), 3)
